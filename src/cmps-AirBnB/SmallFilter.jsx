import { IoSearch } from "react-icons/io5";
export function SmallFilter() {
    return (
        <div className='filter-container flex row flex-start align-center'>
            <button className="filter-button">Anywhere</button>
            <span className="filter-splitter"></span>
            <button className="filter-button">Any week</button>
            <span className="filter-splitter"></span>
            <button className="filter-button filter-add-guests">Add guests</button>
            <button className="mini-search-button"><IoSearch /></button>
        </div>
    )
}
